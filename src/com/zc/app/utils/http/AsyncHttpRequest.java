	...
	private void makeRequest() throws IOException {
		if (!Thread.currentThread().isInterrupted()) {
			try {
				...
				org.apache.http.Header[] requestHeaders = request
						.getAllHeaders();

				ZCLog.i(Tag,
						"requestHeaders length: "
								+ String.valueOf(requestHeaders.length));

				for (int i = 0; i < requestHeaders.length; i++) {
					org.apache.http.Header hd = requestHeaders[i];
					ZCLog.i("Tag", "Header Name: " + hd.getName()
				...
				if (!Thread.currentThread().isInterrupted()) {
					if (responseHandler != null) {
						...
						org.apache.http.Header[] sHeaders = response
								.getAllHeaders();
