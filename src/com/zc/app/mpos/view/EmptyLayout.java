...
public class EmptyLayout {
	 ...
	 * 
	 * @param loadingView
	 *            the layout to be shown when the list is loading
	 */
	 ...
	 * 
	 * @param res
	 *            the resource of the layout to be shown when the list is
	 *            loading
	 */
	 ...
	 * 
	 * @param emptyView
	 *            the layout to be shown when no items are available to load in
	 *            the list
	 */
	 ...
	 * 
	 * @param res
	 *            the resource of the layout to be shown when no items are
	 ...
	 * 
	 * @param errorView
	 *            the layout to be shown when list could not be loaded due to
	 *            some error
	 */
	 ...
	 * 
	 * @param res
	 *            the resource of the layout to be shown when list could not be
	 ...
	 * 
	 * @param animation
	 *            the animation to play when the list is being loaded
	 */
	 ...
	 *            the message
	 * @param messageViewId
	 *            the id of the text view within the loading layout whose text
	 *            will be changed into this message
	 */
	 ...
	 * 
	 * @param loadingButtonClickListener
	 ...
	 * 
	 * @param showEmptyButton
	 *            will a button be shown in the loading view
	 */
	 ...
	 * 
	 * @param loadingViewButtonId
	 *            the ID of the button in the loading view
	 */
	...
	private void refreshMessages() {
		...
		if (mLoadingMessageViewId > 0 && mLoadingMessage != null)
			((TextView) mLoadingView.findViewById(mLoadingMessageViewId))
	...
	private void setDefaultValues() {
		if (mEmptyView == null) {
			mEmptyView = (ViewGroup) mInflater.inflate(R.layout.view_empty,
					null);
		...
		if (mLoadingView == null) {
			mLoadingView = (ViewGroup) mInflater.inflate(R.layout.view_loading,
					null);
			...
			if (mShowLoadingButton && mLoadingViewButtonId > 0
				...
				View loadingViewButton = mLoadingView
		...
		if (mErrorView == null) {
			mErrorView = (ViewGroup) mInflater.inflate(R.layout.view_error,
					null);
	 ...
	 *            the context (preferred context is any activity)
	 * @param listView
	 *            the list view for which this library is being used
	 */
	public EmptyLayout(Context context, PullToRefreshListView listView) {
		mContext = context;
		mInflater = (LayoutInflater) mContext
				.getSystemService(Context.LAYOUT_INFLATER_SERVICE);
		mListView = listView;
